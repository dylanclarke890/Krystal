...
namespace Krys::Gfx
{
  ...
  {
    KRYS_ASSERT(handle.IsValid() && _vertexBuffers.contains(handle), "Invalid vertex buffer handle");
    _vertexBuffers.erase(handle);
  }

  void GraphicsContext::DestroyIndexBuffer(IndexBufferHandle handle) noexcept
  {
    KRYS_ASSERT(handle.IsValid() && _indexBuffers.contains(handle), "Invalid index buffer handle");
    _indexBuffers.erase(handle);
  }

  void GraphicsContext::DestroyShader(ShaderHandle handle) noexcept
  {
    KRYS_ASSERT(handle.IsValid() && _shaders.contains(handle), "Invalid shader handle");
    _shaders.erase(handle);
  }

  void GraphicsContext::DestroyProgram(ProgramHandle handle) noexcept
  {
    KRYS_ASSERT(handle.IsValid() && _programs.contains(handle), "Invalid Program handle");
  ...
  {
    KRYS_ASSERT(handle.IsValid() && _uniformBuffers.contains(handle), "Invalid uniform buffer handle");
