...
#pragma region Init Helpers
  ...
  void Renderer::InitShaders()
  {
    ...
    TextureUnits->SetSamplerUniforms({Shaders.ForwardShading, Shaders.PostProcessing, Shaders.Model});
  }
  ...
  void Renderer::InitFramebuffers()
  {
...

    {
      ...
      Framebuffers.GaussianBlur->SetRenderStepCallback([&]()
                                                       ...
                                                       Context->DrawVertices(6, PrimitiveType::Triangles);
...
#pragma endregion Drawing Cubes
  ...
  void Renderer::DrawModel(Ref<Model> model, Ref<Transform> transform)
  {
    ...
    for (int i = 0; i < model->Meshes.size(); i++)
    {
      ...
      uint32 count = static_cast<uint32>(indexed ? mesh->Indices.size() : mesh->Vertices.size());
      auto PrimitiveType = mesh->PrimitiveType;

      // DepthPasses(indexed, count, PrimitiveType);
      ...
      if (indexed)
        Context->DrawIndices(count, PrimitiveType);
      else
        Context->DrawVertices(count, PrimitiveType);
    }
  }
...
#pragma endregion Skybox
  ...
  void Renderer::Flush()
  {
    ...
    {
      ForwardRender(DefaultVertexArray, VertexCount, true, PrimitiveType::Triangles);
      break;
    }
  ...
  void Renderer::ForwardRender(Ref<VertexArray> vertexArray, uint32 count, bool indexed, PrimitiveType PrimitiveType)
  {
    TextureUnits->Bind();
    TextureUnits->UnbindReserved();
    ...
    // Draw Lights
    {
      ...
      if (IndexCount > 0)
      {
        DefaultVertexBuffer->SetData(Vertices->data(), VertexCount * sizeof(VertexData));
        DefaultIndexBuffer->SetData(Indices->data(), IndexCount);
        Context->DrawIndices(IndexCount, PrimitiveType::Triangles);
      }
    }
  ...
  void Renderer::DepthPasses(bool indexed, uint32 count, PrimitiveType PrimitiveType)
  {
    // Directional
