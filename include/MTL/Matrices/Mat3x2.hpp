...
namespace Krys
{
  ...
  {
    template <IsArithmeticT TComponent>
    struct Matrix<TComponent, 3, 2>
    {
      KRYS_MATRIX_PROPERTIES(3, 2);
    ...
    public:
      ...
      KRYS_MATRIX_TWO_COLUMNS_BITWISE_SHIFT()
      KRYS_MATRIX_TWO_COLUMNS_BITWISE_OR()
      KRYS_MATRIX_TWO_COLUMNS_BITWISE_XOR()
      KRYS_MATRIX_TWO_COLUMNS_BITWISE_NOT()
      KRYS_MATRIX_TWO_COLUMNS_BITWISE_AND()
...
#pragma region Constructors
      ...
      explicit constexpr Matrix(component_t x0, component_t y0, component_t z0, component_t x1,
                                component_t y1, component_t z1) noexcept
          : _values {column_t(x0, y0, z0), column_t(x1, y1, z1)}
      {
      }
